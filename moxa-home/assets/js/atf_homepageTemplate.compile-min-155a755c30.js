 ...
 */
  ...
  /***/ (function (module, __webpack_exports__, __webpack_require__) {
    ...
    /* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, 'initPromotionAnimation', () => initPromotionAnimation);
    $(() => {
      if (!checkCookie()) {
    ...
    function initPromotionAnimation() {
      ...
      if (!overlay) return;
      const delay = overlay.getAttribute('data-overlay-delay') || 0;
      toggleScrollbar();
      setTimeout(() => {
    ...
    function setupPromotionSection() {
      setSectionHeight();
      const resizeListener = attachResizeEvent(setSectionHeight);
    ...
    function setSectionHeight() {
      ...
      const headerHeight = header === null || header === void 0 ? void 0 : header.offsetHeight;
      const promotionSection = document.querySelector('.promotion-section');
      const bgExtension = promotionSection === null || promotionSection === void 0 ? void 0 : promotionSection.querySelector('.promotion-video-bg-extension');
    ...
    function initCloseButton(Listener) {
      const promotionSection = document.querySelector('.promotion-section');
      const promotionBanner = promotionSection && promotionSection.querySelector('.promotion-banner');
      const closeButton = promotionSection && promotionSection.querySelector('.promotion-video-block .close');

      if (closeButton && promotionBanner) {
        closeButton.addEventListener('click', () => {
          const promotionSectionHeight = promotionSection.clientHeight;
          const bannerHeight = (promotionBanner === null || promotionBanner === void 0 ? void 0 : promotionBanner.clientHeight) || 0;
          const videoBlock = promotionSection.querySelector('.promotion-video-block');
          const videoBlockHeight = promotionSectionHeight - bannerHeight;
    ...
    function runOverlayAnimation(overlay) {
      if (!overlay) return;
      const promotionSection = document.querySelector('.promotion-section');
      const duration = overlay && parseInt(overlay.getAttribute('data-overlay-duration')) || 0;
      const text = document.querySelector('.promotion-overlay .promotion-text'); // get duration off css variable and convert to millisecs

      const startDuration = parseInt(getComputedStyle(document.documentElement).getPropertyValue('--promotion-overlay-start-duration')) * 1000 || 0; // overlay enters
    ...
    function writeCookie() {
      ...
      const dateGMTStr = new Date(currentDate.setDate(currentDate.getDate() + 1)).toUTCString();
      document.cookie = 'promotionCookie=1; expires='.concat(dateGMTStr);
    }

    function checkCookie() {
      return document.cookie.split(';').some((item) => item.trim().startsWith('promotionCookie='));
    }
    /***/ }),
  ...
  /***/ (function (module, __webpack_exports__, __webpack_require__) {
    ...
    const jquery_js_exposed = __webpack_require__(0);
    const jquery_js_exposed_default = /* #__PURE__ */__webpack_require__.n(jquery_js_exposed);

    // EXTERNAL MODULE: ./node_modules/slick-carousel/slick/slick.js
    const slick_slick = __webpack_require__(33);

    // EXTERNAL MODULE: ./src/js/utilities/fetch-data.js
    const fetch_data = __webpack_require__(62);
    ...
    window.addEventListener('DOMContentLoaded', () => {
      const standardEmbed = _toConsumableArray(document.querySelectorAll('.iframe-block .kWidgetIframeContainer, .iframe-block .kWidgetPlayBtn'));
    ...
    function loadPlaybackTimeTracking() {
      kWidget.addReadyCallback(/* #__PURE__ */function () {
        const _ref = _asyncToGenerator(/* #__PURE__ */regeneratorRuntime.mark(function _callee(playerId) {
          let kdp; let closestIframeBlock; let postInterval; let recordInterval; let userEntryId; let
            postPlaybackTimeToBackend;
    ...
    function reloadKalturaPlayers() {
      if (typeof kalturaEmbeds !== 'undefined') {
        ...
        ((_kalturaEmbeds = kalturaEmbeds) === null || _kalturaEmbeds === void 0 ? void 0 : _kalturaEmbeds.length) && kalturaEmbeds.forEach((player) => {
          player();
        });
      }
    }
    ...
    jquery_js_exposed_default()(document).ready(() => {
      // vars
      let w = jquery_js_exposed_default()(window).width();
      ...
      var config = {
        ...
        videoResizer: function videoResizer(iframeBox, iframes) {
          const ratio = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : 16 / 9;
          if (!iframes[0]) return;
          const win = iframeBox;
          ...
          iframes.each(function () {
            const current = jquery_js_exposed_default()(this);
      ...
      if (jquery_js_exposed_default()('.hero-carousel .js-modal-trigger[data-modal-id="video"]').length) {
        ...
        window.onYouTubeIframeAPIReady = function () {
          storage.videoItem.each(function () {
            ...
            const ytId = jquery_js_exposed_default()(this).data('video-id');
      ...
      storage.slider.on('beforeChange', (event, slick, currentSlide, nextSlide) => {
        ...
        const videoType = $currentSlide.find('.'.concat(main, '__item')).attr('class').split(' ')[1];
        ...
        if (videoId !== 'img') {
          ...
          switch (videoType) {
            ...
            case '-videoVD':
              if (video !== null) {
                video.play();
                jquery_js_exposed_default()(video).off('ended').on('ended', () => {
                  storage.slider.slick('slickPlay');
                });
              }
            ...
            default:
              ...
              if (kalturaContainer) {
                const kalturaIframeContainer = kalturaContainer.find('.kWidgetIframeContainer');
                const kalturaHasInited = kalturaIframeContainer.length > 0;

                if (kalturaHasInited) {
                  let _kalturaIframeContain; let
                    _kalturaIframeContain2;
      ...
      jquery_js_exposed_default()(window).on('scroll', () => {
        // detect scroll to stop
        if (jquery_js_exposed_default()('.'.concat(main)).length > 0) {
          const scrollTop = jquery_js_exposed_default()('html, body').scrollTop();
          const bannerHeight = jquery_js_exposed_default()('.'.concat(main)).offset().top + jquery_js_exposed_default()('.'.concat(main)).height() * (2 / 3);
          const videoId = config.currentVideoId();
          w = jquery_js_exposed_default()(window).width();
          const $currentSlide = storage.slider.find('.slick-current');
          const videoType = $currentSlide.find('.'.concat(main, '__item')).attr('class').split(' ')[1];
          const video = $currentSlide.find('video').length > 0 ? $currentSlide.find('video')[0] : null; // 偵測若div內容為影片
      ...
      jquery_js_exposed_default()(window).on('resize', () => {
        config.videoResizer(storage.slider, jquery_js_exposed_default()(storage.iframeYoutube), 16 / 9);
        config.videoResizer(storage.slider, jquery_js_exposed_default()(storage.iframeKaltura), 192 / 55);
  ...
  /***/ 410:
  /***/ (function (module, exports, __webpack_require__) {
    const ytCarousel = __webpack_require__(368);

    window.promotion = __webpack_require__(360);

    const video = __webpack_require__(411);
    /***/ }),
  ...
  /***/ (function (module, exports) {
    ...
    (function () {
      window.addEventListener('DOMContentLoaded', () => {
        ...
        const videoModals = _toConsumableArray(document.querySelectorAll('.modal--video'));

        (videoModals === null || videoModals === void 0 ? void 0 : videoModals.length) && videoModals.forEach((modal) => {
          modal.addEventListener('modalClose', (e) => {
            const huodeVid = modal.querySelector('.CCH5playerContainer video');
      ...
      function InitVideo() {
        const videoFrames = [].slice.call(document.querySelectorAll('[data-youtube-vid-id]'));
        ...
        function loadVideos(videoFrames) {
          ...
          videoFrames.forEach((videoFrame, i) => {
            const videoId = videoFrame.getAttribute('data-youtube-vid-id');
            const disableAutoplay = videoFrame.hasAttribute('data-youtube-disable-autoplay');
            const customName = videoFrame.getAttribute('data-youtube-custom-name');
            window[customName || 'vidPlayer'.concat(i)] = new YT.Player(videoFrame, {
              height: '100%',
              width: '100%',
              videoId,
              playerVars: {
              ...
              events: {
                ...
                onStateChange: function onStateChange(_ref) {
                  const state = _ref.data;
                  const player = _ref.target;
